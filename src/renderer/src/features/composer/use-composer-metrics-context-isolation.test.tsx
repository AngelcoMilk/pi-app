...
describe('useComposerMetrics context isolation', () => {
  ...
  it('clears the old preview while the next session is loading', async () => {
    const next = deferred<{
    ...
    vi.mocked(ipcClient.invoke).mockImplementation((channel, request) => {
      ...
      if (sessionFile === '/sessions/b.jsonl') return next.promise
      return Promise.resolve({
        preview: {
          sessionFile: '/sessions/a.jsonl',
          messageCount: 1,
    ...
    await waitFor(() => expect(result.current.contextPreview?.estimatedChars).toBe(10))
