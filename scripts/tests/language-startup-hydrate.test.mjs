...
describe('saved language startup hydration', () => {
  it('should_hydrate_saved_language_before_first_react_render', () => {
    ...
    const awaitedHydration = mainSource.match(/await Promise\.allSettled\(\[([\s\S]*?)\]\)/)
    const hydrationPosition = awaitedHydration?.index ?? -1
    const renderPosition = mainSource.indexOf('ReactDOM.createRoot')

    assert.ok(awaitedHydration, 'renderer bootstrap must await startup hydration')
    assert.match(
      awaitedHydration[1],
      /hydrateLanguageFromSettings\(\)/,
      'saved language hydration must be part of the awaited startup barrier',
    )
